...
function App() {
  ...
  const sortOptions = ["title", "description", "price", "city","month_date","year_date"];
  ...
  const loadUsersData = async () => {
    return await axios.get("http://localhost:5000/events")
  ...
  return (
    <MDBContainer>
      ...
      <MDBCol size="8"> 
        <h5> Trier par :</h5>
        <select style={{borderRadius:"2px", height:"35px"}}
        onChange={handleSort}
        value={sortValue}>
                ...
                data.map((item, index) => (
                  <MDBTableBody key={index}>
                    <tr>
                      ...
                      <td>{item['street']}<br/>
                      {item['city']}</td>
                      <td>{item['day_date']}<br/>{item['month_date']}<br/>
                      {item['year_date']}
                      </td>
                    </tr>
